...
def get_args():
    ...
    parser.add_argument("-n", "--njobs", type=int, default=8, help="Number of parallel jobs")
...
def main():
    ...
    results = Parallel(n_jobs=args.njobs)(delayed(process)(file_name, dft) for file_name, dft in tqdm(df.groupby("ImageId")))
    #for file_name, dft in tqdm(df.groupby("ImageId")):
    #    process(file_name, dft)
    print(results)
